...
class MCPGitVerificationTests:
    ...
    async def mcp_client(self):
        ...
        cmd = [
            "python", "-m", "mcp_server_git.server_simple", 
            "--test-mode"
        ]
    ...
    async def test_phase_1_basic_git_operations(self, mcp_client, test_repo):
        ...
        response = await mcp_client.send_request("tools/call", {
            "name": "git_status",
            "arguments": {"repo_path": str(test_repo)}
        })
        
        assert "error" not in response
        assert "result" in response
        status_content = response["result"][0]["text"]
        assert "modified:" in status_content.lower() or "changes not staged" in status_content.lower()
    ...
    async def test_phase_2_github_api_operations(self, mcp_client):
        ...
        if not has_github_token:
            ...
            is_graceful_failure = "No GitHub token" in result_text or "404" in result_text or "Failed to" in result_text
    ...
    async def test_phase_3_advanced_git_operations(self, mcp_client, test_repo):
        ...
        assert "security" in security_content.lower() or "validation" in security_content.lower()
    ...
    async def test_phase_4_error_handling_and_edge_cases(self, mcp_client):
        ...
        assert "❌" in error_content or "error" in error_content.lower() or "not" in error_content.lower()
        ...
        assert "result" in response
        error_content = response["result"][0]["text"]
        assert "❌" in error_content or "404" in error_content or "failed" in error_content.lower()
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            assert "result" in response
            error_content = response["result"][0]["text"]
            assert "❌" in error_content or "error" in error_content.lower() or "invalid" in error_content.lower()
        ...
        response = await mcp_client.send_request("tools/call", {
            "name": "git_status",
            "arguments": {"repo_path": str(Path.cwd())}
        })
    ...
    async def test_comprehensive_verification_report(self, mcp_client, test_repo):
        ...
        try:
            ...
            verification_results["basic_git_ops"] = "result" in response and "error" not in response
            ...
            verification_results["advanced_ops"] = "result" in response and "error" not in response
            ...
            print(f"  ✅ Error Handling: {'PASS' if verification_results['error_handling'] else 'FAIL'}")
            ...
            assert verification_results["github_api"], "GitHub API must respond (even if no token)"
