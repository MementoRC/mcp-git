...
from mcp_server_git.optimizations import enable_validation_cache, disable_validation_cache, clear_validation_cache, get_validation_cache_stats
...
async def test_message_throughput_ping(benchmark_session_manager, mock_client):
    ...
    await benchmark_session_manager.create_session(mock_client.session_id, user="benchmark_user")
    ...
    logger.info(f"Ping Throughput: {throughput:.2f} messages/sec ({messages_sent} messages in {duration:.2f}s)")
...
async def test_message_throughput_operations(benchmark_session_manager, mock_client):
    ...
    logger.info(f"Operation Throughput: {throughput:.2f} operations/sec ({operations_completed} operations in {duration:.2f}s)")
...
async def test_validation_caching_effectiveness(memory_monitor):
    ...
    for i in range(VALIDATION_CACHE_TEST_MESSAGES):
        if i % 10 == 0:
            ...
            test_messages.append({
                ...
                "params": {"requestId": "repeated_request", "reason": "Test cancellation"}
            })
        else:
            # Unique message
            test_messages.append({
                "jsonrpc": "2.0", 
                "method": "notifications/cancelled",
                "params": {"requestId": f"request_{i}", "reason": f"Cancellation {i}"}
            })
    ...
    performance_ratio = no_cache_duration / with_cache_duration if with_cache_duration > 0 else 1.0
    ...
    assert cache_stats.get("hits", 0) + cache_stats.get("misses", 0) > 0, "Cache should show some activity"
    ...
    assert with_cache_duration < 1.0, "Cached operations should complete in reasonable time"


@pytest.mark.benchmark
@pytest.mark.ci_skip  # Too intensive for CI
@pytest.mark.asyncio
async def test_realistic_mixed_workload_performance(benchmark_session_manager, memory_monitor):
    ...
    class MockMCPClient:
        ...
        async def start_operation(self, operation_id):
            if not self.connected:
                raise RuntimeError("Client not connected")
            self.message_count += 1
            return {"type": "operation_started", "id": str(uuid.uuid4()), "operation_id": operation_id}
        
        async def cancel_operation(self, operation_id):
            if not self.connected:
                raise RuntimeError("Client not connected")
            self.message_count += 1
            return {"type": "operation_cancelled", "id": str(uuid.uuid4()), "operation_id": operation_id}
    ...
    for i, client in enumerate(clients):
        ...
        session = await benchmark_session_manager.create_session(client.session_id, user=f"perf_user_{i}")
    ...
    logger.info(f"Mixed Workload Performance: {throughput:.2f} msg/sec, {error_rate:.2%} errors")
