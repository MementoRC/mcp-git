...
class MCPBehaviorValidator:
    ...
    def create_test_repository(self) -> Path:
        ...
        try:
            ...
            subprocess.run(["git", "config", "user.name", "MCP Validator"], cwd=repo_path, check=True)
            subprocess.run(["git", "config", "user.email", "validator@example.com"], cwd=repo_path, check=True)
            ...
            subprocess.run(["git", "commit", "-m", "Add branch file"], cwd=repo_path, check=True)
            subprocess.run(["git", "checkout", "master"], cwd=repo_path, check=True)
            
            self.log_result("create_test_repository", True, "Test repository created successfully")
            return repo_path
            
        except subprocess.CalledProcessError as e:
            self.log_result("create_test_repository", False, f"Failed to create test repository: {e}")
            raise
    ...
    def test_notification_models(self) -> bool:
        ...
        try:
            ...
            self.log_result("notification_models", False, f"Notification model test failed: {e}")
            return False
    ...
    def test_unknown_notification_handling(self) -> bool:
        ...
        try:
            ...
            self.log_result("unknown_notification_handling", True, f"Unknown notification handling: {e}")
            return True
    ...
    def test_server_startup(self) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.log_result("server_startup", False, f"Server help failed: {result.stderr}")
                return False
    ...
    def test_git_operations_integration(self, repo_path: Path) -> bool:
        ...
        try:
            ...
            self.log_result("git_operations_integration", True, "Git operations integration test passed")
            return True
            
        except Exception as e:
            self.log_result("git_operations_integration", False, f"Git integration test failed: {e}")
            return False
        ...
        try:
            ...
            if errors_handled >= 0:  # At least some error handling
                self.log_result(
                    ...
                    {"errors_handled": errors_handled, "total_tests": len(malformed_notifications)}
                )
                return True
            else:
    ...
    def test_protocol_compliance(self) -> bool:
        ...
        try:
            ...
            self.log_result("protocol_compliance", True, "Basic protocol compliance verified")
            return True
            
        except Exception as e:
            self.log_result("protocol_compliance", False, f"Protocol compliance test failed: {e}")
            return False
...
def main():
    ...
    parser.add_argument("--test-repo", type=Path, help="Path to test repository (will create if not exists)")
