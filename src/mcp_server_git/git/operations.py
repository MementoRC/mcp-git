...
def git_commit(repo: git.Repo, message: str, gpg_sign: bool = False, gpg_key_id: Optional[str] = None) -> str:
...
def git_log(repo: git.Repo, max_count: int = 10, oneline: bool = False, graph: bool = False, format_str: Optional[str] = None) -> str:
...
def git_push(repo: git.Repo, remote: str = "origin", branch: Optional[str] = None, set_upstream: bool = False, force: bool = False) -> str:
...
def git_diff_branches(repo: git.Repo, base_branch: str, compare_branch: str) -> str:
    ...
    try:
        ...
        all_branches = [branch.name for branch in repo.branches] + [ref.name.split('/')[-1] for ref in repo.remote().refs]
...
def git_merge(repo: git.Repo, source_branch: str, strategy: str = "merge", message: Optional[str] = None) -> str:
